"""
MALLORN v53: LightGBM + TDE-Only GP Data Augmentation

Strategy revision based on v50/v51 results:
- v50 (XGBoost, augment all): OOF F1=0.6388, LB F1=0.6721
- v51 (LightGBM, augment all): OOF F1=0.5967
...
- Different ensemble potential vs XGBoost
- May benefit differently from TDE-focused augmentation
...
print("=" * 80, flush=True)
print("MALLORN v53: LightGBM + TDE-Only GP Data Augmentation", flush=True)
print("=" * 80, flush=True)
...
print(f"   v51 (LightGBM, augment all): OOF F1=0.5967", flush=True)
...
# ====================
# 4. TRAIN LIGHTGBM ON TDE-AUGMENTED DATA
# ====================
print("\n4. Training LightGBM with 5-fold CV on TDE-augmented data...", flush=True)

y_combined = combined_targets

# LightGBM hyperparameters (same as v46)
lgb_params = {
    'objective': 'binary',
...
submission_path = base_path / 'submissions/submission_v53_lgb_tde_aug.csv'
...
with open(base_path / 'data/processed/v53_artifacts.pkl', 'wb') as f:
    pickle.dump(artifacts, f)

print("\n" + "=" * 80, flush=True)
print(f"MALLORN v53 (LightGBM + TDE-Only Augmentation) Complete: OOF F1 = {best_f1:.4f}", flush=True)
print(f"v46 (LightGBM baseline): OOF F1 = {v46['oof_f1']:.4f}, LB F1 = 0.60", flush=True)
print(f"v51 (LightGBM, augment all): OOF F1 = 0.5967", flush=True)
print(f"Training samples: {len(train_meta)} -> {len(combined_meta)} (TDEs: {n_tdes} -> {n_tdes_combined})", flush=True)

print("\nKey Insight: TDE-only data augmentation (LightGBM)", flush=True)
...
print("  - Leaf-wise growth may benefit differently than XGBoost", flush=True)
print("  - Good ensemble potential with v52 (XGBoost)", flush=True)

print("\nNEXT STEP: Test v53 on leaderboard", flush=True)
print("Then analyze ensemble potential between v52 + v53", flush=True)
print("=" * 80, flush=True)
